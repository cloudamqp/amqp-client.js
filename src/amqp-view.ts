...
export default class AMQPView extends DataView {
  ...
  setUint64(byteOffset: number, value: number, littleEndian = false): void {
    this.setBigUint64(byteOffset, BigInt(value), littleEndian)
  }

  getInt64(byteOffset: number, littleEndian = false): number {
    return Number(this.getBigInt64(byteOffset, littleEndian))
  }

  setInt64(byteOffset: number, value: number, littleEndian = false): void {
  ...
  getLongString(byteOffset: number, littleEndian = false): [string, number] {
  ...
  setLongString(byteOffset: number, string: string, littleEndian = false): number {
    const encoder = new TextEncoder()
    const utf8 = encoder.encode(string)
  ...
  getProperties(byteOffset: number, littleEndian = false): [AMQPProperties, number] {
    let j = byteOffset
  ...
  setProperties(byteOffset: number, properties: AMQPProperties, littleEndian = false): number {
    let j = byteOffset
    let flags = 0
  ...
  getTable(byteOffset: number, littleEndian = false): [Record<string, unknown>, number] {
    const table = {}
    let i = byteOffset
  ...
  setTable(byteOffset: number, table: object, littleEndian = false): number {
  ...
  // eslint-disable-next-line @typescript-eslint/ban-types
  getField(byteOffset: number, littleEndian = false): [string|number|boolean|object, number] {
    let i = byteOffset
    const k = this.getUint8(i); i += 1
    const type = String.fromCharCode(k)
    // eslint-disable-next-line @typescript-eslint/ban-types
    let v: string|number|boolean|object
    let len: number
    switch (type) {
  ...
  setField(byteOffset: number, field: any, littleEndian = false): number {
    let i = byteOffset
    switch (typeof field) {
      case "string":
  ...
  getArray(byteOffset: number, littleEndian = false):[any[], number] {
  ...
  setArray(byteOffset: number, array: any[], littleEndian = false): number {
