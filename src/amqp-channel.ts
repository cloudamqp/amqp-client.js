...
export class AMQPChannel {
  ...
  constructor(connection: AMQPBaseClient, id: number) {
    ...
    this.onerror = (reason: string) => {
      ...
      this.connection.onerror(
        new AMQPError(`Channel ${this.id} closed: ${reason}`, this.connection),
      );
    };
  }
  ...
  async basicPublish(
    ...
    const bodyFrameCount = Math.ceil(
      body.byteLength / (this.connection.frameMax - 8),
    );
    ...
    if (this.confirmId) {
      ...
      return sendFrames
        .then(() => wait4Confirm)
        .finally(() => this.connection.bufferPool.push(buffer));
    } else {
      return sendFrames
        .then(() => 0)
        .finally(() => this.connection.bufferPool.push(buffer));
