...
test("can set frameMax", async () => {
  const amqp = new AMQPWebSocketClient({ url: "ws://127.0.0.1:3002/ws/amqp", frameMax: 16*1024 })
  const conn = await amqp.connect()
  const ch = await conn.channel()
  await ch.confirmSelect()
  const q = await ch.queue("")
...
test("can handle heartbeats", async () => {
  const amqp = new AMQPWebSocketClient("ws://127.0.0.1:3002/ws/amqp?heartbeat=1")
  const conn = await amqp.connect()
...
test("has an onerror callback", async () => {
  ...
  const ch = await conn.channel()
  conn.onerror = (err) => {
    expect(err.message).toMatch(/invalid exchange type/)
  }
...
test("onerror is not called when conn is closed by client", async () => {
  ...
  const conn = await amqp.connect()
  conn.onerror = vi.fn(
    () => assert.fail("onerror should not be called when gracefully closed")
  )
  await conn.close()
...
test("will throw on too large headers", async () => {
  const amqp = new AMQPWebSocketClient({ url: "ws://127.0.0.1:3002/ws/amqp", frameMax: 4096 })
  const conn = await amqp.connect()
  const ch = await conn.channel()
