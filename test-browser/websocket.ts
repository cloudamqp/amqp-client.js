...
test("can declare an exchange", async () => {
  ...
  await expect(ch.basicPublish(name, "rk", "body")).rejects.toThrow(
    /NOT_FOUND/,
  );
});
...
test("can publish and consume msgs with large headers", async () => {
  ...
  await q.publish("a".repeat(8000), {
    headers: { long: Array(100).fill("a") },
  });
...
test("will throw when headers are too long", async () => {
  ...
  const q = await ch.queue();
  await expect(
    q.publish("a".repeat(8000), { headers: { long: "a".repeat(9000) } }),
  ).rejects.toThrow();
});
...
test("will throw on too large headers", async () => {
  ...
  await expect(
    ch.basicPublish("", "", null, { headers: { a: "x".repeat(9000) } }),
  ).rejects.toThrow(RangeError);
});
